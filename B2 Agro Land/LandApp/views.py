...
def getContract():
    ...
    deployed_contract_address = '0xB50Bb6b655f7f214076003c171C82Ee633e6fFfa' #contract address
...
def ViewRequestStatus(request):
    if request.method == 'GET':
        ...
        output += '<th><font size="3" color="black">IPFS Transaction Hashcode</font></th>'
        output += '<th><font size="3" color="black">Request Date</font></th>'
        output += '<th><font size="3" color="black">AI Detected Transaction Status</font></th>'
        output += '<th><font size="3" color="black">Seller Decision</font></th>'
        output += '</tr>'
        for i in range(len(transactionList)):
            ...
            if tl[0] == username:
                output += '<tr>'
                output += '<td><font size="3" color="black">'+str(tl[0])+'</font></td>'
                output += '<td><font size="3" color="black">'+tl[1]+'</font></td>'
                output += '<td><font size="3" color="black">'+status[0]+'</font></td>'
                if status[1] == 'Fraud':
                    output += '<td><font size="3" color="red">'+status[1]+'</font></td>'
                else:
                    output += '<td><font size="3" color="green">'+status[1]+'</font></td>'
                decision = "Pending"
                if len(status) == 3:
                    decision = status[2]
                output += '<td><font size="3" color="black">'+decision+'</font></td>'
                output += '</tr>'
...
def AcceptRejectAction(request):
    if request.method == 'GET':
        ...
        msg = contract.functions.updateStatus(int(index), tl[2]).transact()
        web3.eth.waitForTransactionReceipt(msg)

        # 🔹 Refresh latest records from blockchain
        getTransactionList()
